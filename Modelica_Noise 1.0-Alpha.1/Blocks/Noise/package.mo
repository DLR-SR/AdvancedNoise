...
numbers in an <b>efficient</b> way. Reproducibility is important when designing control systems,
...
computed noise has changed the behaviour of the controlled system). Typical noise blocks slow down
simulations, say, by a factor of 10 .. 100. This is different with the blocks of this sublibrary:
The default setting
...
<tr><td> TimeBasedNoise </td>
    <td> The random values provided at the output of a TimeBasedNoise instance
         depend (a) on the <b>actual time</b> instant, (b) on the instance name,
         and (c) on the settings of the respective instance (as well as the
         seetings in globalSeed, see above). By default, two or more instances
         produce different, uncorrelated noise at the same time instant.</td></tr>

<tr><td> SignalBasedNoise </td>
    <td> The random values provided at the output of a SignalBasedNoise instance
         depend (a) on the <b>actual value of the input</b> signal,
         (b) on the instance name, and (c) on the settings of the respective
         instance (as well as the seetings in globalSeed, see above).
         By default, two or more instances
         produce different, uncorrelated noise at the same value of the input
         signal. The default input signal is \"time\", and then the behavior of a
         SignalBasedNoise instance is similar to a TimeBasedNoise instance. </td></tr>
...
<tr><td> globalSeed.random() </td>
    <td> Function random() is defined inside the globalSeed component.
         ...
         random value is returned. Since this is an impure function, it can only
         ...
         to implement a special block.
         The drawback of this function is that even small changes of a model
         may change the sequence of the random() calls and then reproducibility
         is lost.</td></tr>
</table>
</p></blockquote>
...
<p>
The uniform random numbers in the range 0.0 .. 1.0 are transformed to a desired band 
and random number distribution by selecting an appropriate
...
In the blocks of this sublibrary, the desired truncated disribution can be selected,
but also a user-defined distribution.
</p>


<h4>Efficient Simulation by Event-Free Interpolation</h4>

<p>
Efficient simulation of noise blocks is reached in different ways by the blocks.
...
<li> If a constant interpolation method is selected between grid points, events are
     generated at the grid points and simulation will be slow, but according to the
     traditional approach. </li>
<li> If a continuous interpolation method is selected (for example linear
     interpolation of the grid points), then basically no events are generated.
     Experience shows that simulation can then be much faster.
     </li>
</ul>

<p>
An example is shown in the next figure, where random numbers are drawn periodically
every 0.1 s. When using a constant interpolation, an event is generated at every grid
point. When using linear or smooth interpolation, no events are generated.
</p>
...
Instead, smooth interpolation (smoothNoise.y) may violate the band
...
<p>
Other properties of the interpolation are demonstrated in example
<a href=\"Modelica_Noise.Blocks.Examples.NoiseExamples.Interpolation\">NoiseExamples.Interpolation</a>. 
In the article
</p>

<p><blockquote>
v.d.Linden F.L.J, A. Kl&ouml;ckner, D. Zimmer (2015): <b>Effects of Event-Free Noise Signals on
Continuous-Time Simulation Perfomance</b>. MathMod'2015 Conference.
</blockquote></p>

<p>
The effect of interpolated, event-free noise signals is analyzed at hand of
several examples, different integration methods and relative tolerances. A typical 
result is shown in the next diagram (a Noise block is used as input to a low pass
filter of order 50) using the DASSL integrator:
</p>

<p><blockquote>
<img src=\"modelica://Modelica_Noise/Resources/Images/Blocks/Noise/EventEffect.png\">
</blockquote></p>

<p>
The selected relative tolerance is used as x-axis, whereas the number of 
evaluations of the model is shown at the y-axis (and is an indicator of the
simulation efficiency). The dashed lines are the result of constant, linear,
and sinc interpolation where an event is generated at every grid point.
Solid lines represent simulations where no event is generated at grid points.
As can be seen, the number of function evaluations
can be considerably smaller with event-free noise if not too tight relative tolerances
are selected. Practical experience with real-world models show that event-free
noise can considerably reduce simulation time.
</p>

<p>
One approach to implement event-free noise is described in the article
</p>

<p><blockquote>
Kl&ouml;ckner A., v.d. Linden F.L.J., and Zimmer D. (2014).
...
<p>
This approach is used by the SignalBasedBlock: The value of the input signal is
used as seed, and whenever a random number shall be drawn, the random number generator
is newly initialized with this seed (this is possible because the xorshift generators
generate statistically releveant random numbers already after drawing only a 
few random numbers. Whenever a random number is required, random numbers are newly
drawn at the defined grid and then interpolated for the desired input signal value.
</p>

<p>
A slightly different approach is used by the TimeBasedBlock: Time events are generated
at a few underlying grid points and an internal buffer of a desired size (say 100 elements)
is filled with random numbers between such grid points. For the duration represented by the bufffer,
no time events are generated and the output of the block is computed by interpolation in the buffer.
</p>
</html>", revisions="<html>
<p>
<table border=1 cellspacing=0 cellpadding=2>
