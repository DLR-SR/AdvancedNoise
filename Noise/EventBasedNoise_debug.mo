within Noise;
block EventBasedNoise_debug "A noise generator based on discrete time events"
...
initial algorithm
  state := (1:stateSize) + fill(localSeed,stateSize);
  // This works:
  (r,state) := distribution(generator=Noise.Generators.LinearCongruentialGenerator, stateIn=state);
...
equation
  when sample(startTime,samplePeriod) then
    ...
    // This would work:
    // (r,state) =  distribution(generator=Noise.Generators.LinearCongruentialGenerator, stateIn=pre(state));
  end when;
