...
<p>
    ...
    algorithm
       ...
       // check Bates
       y1 := Distributions.Bates.density(u,-1,2);
       y2 := Distributions.Bates.cumulative(u,-1,2);
       y3 := Internal.derTwoSided(u,y2);
       ...
       y1 := Distributions.Bates.quantile(u1,-1,2);
       u2 := Distributions.Bates.cumulative(y1,-1,2);
       err :=max(abs(u1 - u2));
