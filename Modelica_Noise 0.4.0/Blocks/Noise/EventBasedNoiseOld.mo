...
block EventBasedNoiseOld
  ...
  parameter Real y_off = 0.0
    "Output value if time < startTime or enableNoise=false"
    annotation(Dialog(tab="Advanced"));
  parameter Integer sampleFactor(min=1)=100
    "Events only at samplePeriod*sampleFactor (if non-constant interpolation)"
    annotation(Evaluate=true,Dialog(tab="Advanced", enable=enableNoise));

  parameter Boolean useGlobalSeed = true
    "= true: use global seed, otherwise ignored it"
  ...
  parameter Boolean useAutomaticLocalSeed = true
    "= true: use instance name hash as local seed, otherwise use fixedLocalSeed"
    annotation(choices(checkBox=true),Dialog(tab="Advanced",group = "Initialization",enable=enableNoise));
  ...
  replaceable package interpolation =
    ...
    "Interpolation method in equidistant grid of raw random numbers"
...
protected
  outer GlobalSeed globalSeed "Definition of global seed via inner/outer";
