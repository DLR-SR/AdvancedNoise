...
<tr><td> distribution </td>
    <td> Defines the random number distribution to map random numbers
         ...
         The distribution function is called at every time instant where an output value
         of the block has to be computed. As a result, a distribution may depend also on
         other variables of a model.</td></tr>
